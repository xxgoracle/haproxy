...
static inline void session_add_idle_conn(struct session *sess, struct connection *conn, void *target)
{
	if (sess->resp_conns >= sess->fe->max_out_conns) {
		struct server *srv = objt_server(conn->target);
		conn->owner = NULL;
		srv_add_to_idle_list(srv, conn);
		return;
	}
	session_add_conn(sess, conn, target);
}
